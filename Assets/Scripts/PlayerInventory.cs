...
public class PlayerInventory : Inventory
{
    ...
    [Command]
    public void CmdSwapInventoryTrash(int inventoryIndex)// Swap Item to Trash
    {
            ...
            0 <= inventoryIndex && inventoryIndex < slots.Count)
        {
            ...
            if (slot.amount > 0 && slot.item.droppable) //&& !slot.item.summoned)
            {
                //if near any lootbag
                if (playerLooting.CanLoot() && !playerLooting.currentloot.IsSoulBound() && playerLooting.currentloot.CanAdd(slot.item, slot.amount))
                {
                    playerLooting.currentloot.Add(slot.item, slot.amount);
                }
                else
                {
                    ...
                    lootbag.GetComponent<LootBag>().AddLoot(slot.item , slot.amount);
                }
    ...
    //Drag to trash now drop item instead.
    /*
    [Command]
    public void CmdSwapTrashInventory(int inventoryIndex)// Swap Trash to Item
    {
        if (InventoryOperationsAllowed() &&
            0 <= inventoryIndex && inventoryIndex < slots.Count)
        {
